...
function buildEmailHtml(form: any, items: any[], total: number) {
  ...
  const vatRow = form.vat
    ? '<p style="font-size:12px;color:#94a3b8;margin:4px 0 0">P.IVA: ' + form.vat + '</p>'
    : ''
  ...
  if (briefingItems.length > 0) {
    const rows = briefingItems.map((i: any) => {
      const fields = Object.entries(i.briefing).map(([k, v]) =>
        '<div style="margin-bottom:4px">' +
        '<span style="font-size:11px;color:#94a3b8;min-width:120px;display:inline-block;text-transform:capitalize">' + String(k).replace(/_/g, ' ') + '</span>' +
        '<span style="font-size:12px;color:#334155;font-weight:500">' + String(v) + '</span>' +
        '</div>'
      ).join('')
    ...
    briefingHtml =
      ...
      '<p style="font-size:10px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#94a3b8;margin:0 0 12px">✏️ Briefing compilato</p>' +
      rows +
      '</div>'
  }
  ...
  return '<!DOCTYPE html><html><head><meta charset="utf-8"/></head>' +
    ...
    itemRows +
    '<tr><td style="padding:14px 0 0;font-size:14px;font-weight:700;color:#0f172a">Totale</td>' +
    '<td style="padding:14px 0 0;font-size:22px;font-weight:800;color:#4dd9c0;text-align:right">€' + total + '</td></tr>' +
    '</table>' +
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error } = await resend.emails.send({
      ...
      html: buildEmailHtml(form, items, total),
    })
